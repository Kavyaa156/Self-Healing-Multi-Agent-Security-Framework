...
from diagnosis.attribution import attribute_root_cause
...
def build_diagnosis(
    ...
    [Phase 5 Novelty #1] root_cause_agent/root_cause_step now reflect
    the TRUE origin of the failure (via attribute_root_cause), not just
    the step where the failure happened to be observed. This closes the
    schema gap noted below -- root_cause_step now exists.
    """
    failure_type = classify_failure(score, graph, event)
    attribution = attribute_root_cause(score, graph, event, failure_type)
    ...
    if failure_type == "F4" and len(attribution.propagation_chain) > 1:
        description = (
            f"{description} Root cause traced to agent "
            f"'{attribution.root_cause_agent}' at step "
            f"{attribution.root_cause_step} "
            f"({len(attribution.propagation_chain)}-node propagation "
            f"chain, confidence={attribution.confidence:.2f})."
        )
    ...
    return FailureDiagnosis(
        ...
        propagation_chain=attribution.propagation_chain,
