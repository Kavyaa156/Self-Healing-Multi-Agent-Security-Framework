from schemas_mock.events import DEFAULT_WEIGHTS
from tests.evaluate import build_sample_task_set, run_evaluation


def test_run_evaluation_produces_report_with_expected_shape():
    tasks, fault_labels = build_sample_task_set(n_per_type=3)
...
def test_f1_and_f3_reliably_caught_by_r_threshold_alone():
...
def test_f2_detection_improves_with_api_orchestration_weights():
    # KNOWN FINDING (see README): F2/F4 only degrade E. Under
    # multi_step_reasoning weights (E weight=0.2), a cratered E alone often
    # does NOT push R below theta=0.65 -- this is real weight-driven
    # behavior, not a bug, and is exactly why Section 10 asks for
    # per-type accuracy reporting ("shows which failure types the C/S/E
    # setup is weakest at catching"). Switching to api_orchestration
    # weights (E weight=0.5) should recover F2 detection.
    tasks, fault_labels = build_sample_task_set(n_per_type=5)
    weak_weights_report = run_evaluation(
        tasks, fault_labels, weights=DEFAULT_WEIGHTS["multi_step_reasoning"]
    )
    strong_weights_report = run_evaluation(
        tasks, fault_labels, weights=DEFAULT_WEIGHTS["api_orchestration"]
    )
    assert (
